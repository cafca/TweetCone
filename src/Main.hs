...
readJSON :: IO (Maybe [Ts.Trending])
readJSON = do
    c <- S.readFile (tcBaseDir ++ dataFileName)
...
queryTree sqs trEntry = node
    (fmap (\ sq -> statusTree (tweets sq) (entryFromQuery sq))
        sqs)
    trEntry
...
trendingTree (Just ts) = rootNode $
    fmap (\ t -> queryTree (trends t) (entryFromText $ Ts.as_of t))
    ts


main = do
    putStrLn "Loading last TweetCone from disk.."
    j <- readJSON
        ...
        go bearerToken = do
            ...
            case mts of
                ...
                (Just ts) -> do
                    ...
                    writeFile (tcBaseDir ++ dataFileName) (B.unpack $ encodePretty ts')
